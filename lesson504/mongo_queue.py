...
class MongoQueue:
    ...
    def __init__(self, client=None, timeout=300):
        ...
        self.db = self.client['python_practice']
    ...
    def push(self, url):
        ...
        """
        try:
            self.col.insert_one({'_id': url, 'status': self.OUTSTANDING})
    ...
    def pop(self):
        ...
        """
        record = self.col.find_and_modify(
    ...
    def peek(self):
        record = self.col.find_one({'status': self.OUTSTANDING})
        if record:
            return record['_id']

    def complete(self, url):
        self.col.update_one({'_id': url}, {'$set': {'status': self.COMPLETE}})
